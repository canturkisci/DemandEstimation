...
#CONFIG CONSTS
my $periodSecs = 30; # will run every 60s
...
# Build Host / PS inventory from the selected hosts for top ten processes
sub BuildPsInventory 
{
	my @vmEntitiesOnHost;
	my $hostNo;
	my $vmNo;
	...
	for ($hostNo = 0; $hostNo <= $#hostArray; $hostNo++) {
		my $vmString = "";
		open(kvmCommands, " top -b -n1 | head -n 17 | tail -n 10 | ")  or die "Could not open kvmCommands: $!\n";
		while ($line = <kvmCommands>) {
			#if ($line =~ /\.*(qemu-kvm)\.*/) {
				$line =~ s/^\s+|\s+$//; #trim leading/trailing whitespace
				@fields = split(/\s+/, $line);
				#for ($i = 0; $i <= $#fields; $i++) {
					#if ($fields[$i] eq "-name") {
						#printf STDOUT ("$line $fields[0] $fields[1]\n");
						# JUST GET THE PID AS NAME (first field)
						$vmString = $vmString . "$fields[0] " ;
						#printf STDOUT ("$vmString \n");
					#}
				#}
			#}
...
sub GetVmThreadIds
{
	...
	for ($hostNo = 0; $hostNo <= $#hostArray; $hostNo++) {
		open(kvmThreads, " ps -eL -o pid,tid,cmd |")  or die "Could not open kvmThreads: $!\n";
...
#			if ($line =~ /\.*(qemu-kvm)\.*/) {
				...
				for ($vmNo = 0; $vmNo <= $#vmArray; $vmNo++) {
					if ($currPid eq $vmArray[$vmNo]) {
						#MY ASSUMPTION:
						# If PID==TID --> Master thread, if PID<TID --> VCPU Thread
						printf "$currPid $currTid\n";
...
sub PrepLogFileName() #Returns log file name as string
{
  ...
  my $finalLogFileName = "psDemandEstimator" . $logDate . ".log";
